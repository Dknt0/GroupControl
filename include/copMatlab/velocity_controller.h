...
class Car {
 public:
  Car(int clientID, std::string name)
      : nh("~"), m_clientID(clientID), m_name(name) {
    simxGetObjectHandle(clientID, ('/' + name).c_str(), &m_carHandel,
    ...
    m_targetSub = nh.subscribe<geometry_msgs::Twist>(
        m_name + "/target", 10,
        std::bind(&Car::cb_target, this, std::placeholders::_1));
    m_velocitySub = nh.subscribe<geometry_msgs::Twist>(
        m_name + "/veolcity", 10,
        std::bind(&Car::cb_velocity, this, std::placeholders::_1));
  }
  ...
  Car(int clientID, std::string name, std::string avoidCarName)
    ...
    simxGetObjectHandle(clientID, ('/' + name).c_str(), &m_carHandel,
                        simx_opmode_blocking);
    simxGetObjectHandle(clientID, ('/' + avoidCarName).c_str(),
                        &m_avoidCarHandel, simx_opmode_blocking);
    m_avoid = true;
  ...
  void setVelocity(const double linear_velocity,
                   const double angular_velocity) {
    simxSetFloatSignal(m_clientID, (simxChar*)m_LV_signal.c_str(),
                       linear_velocity, simx_opmode_oneshot_wait);
    simxSetFloatSignal(m_clientID, (simxChar*)m_AV_signal.c_str(),
                       angular_velocity, simx_opmode_oneshot_wait);
  }
 ...
 private:
  void thread_getPose() {
    while (ros::ok()) {
      ...
      {
        std::unique_lock<std::mutex>(mlock_pose);
        simxGetObjectPosition(m_clientID, m_carHandel, -1, ms_position,
                              simx_opmode_blocking);
        simxGetObjectOrientation(m_clientID, m_carHandel, -1, ms_orientation,
                                 simx_opmode_blocking);
        pose.linear.x = ms_position[0];
        pose.linear.y = ms_position[1];
        pose.angular.z = angles::normalize_angle(ms_orientation[2] - M_PI_2);
      }
      // std::cout << "\rPose: [" << ms_position[0] << ", " << ms_position[1] <<
      // ", " << ms_position[2] << ", "
      //                        << ms_orientation[0] << ", " <<
      //                        ms_orientation[1] << ", " << (ms_orientation[2]
      //                        - M_PI_2)
      //                        << "]        " << std::flush;
      m_posePub.publish(pose);
      if (m_avoid == true) {
        std::unique_lock<std::mutex>(mlock_avoidPose);
        simxGetObjectPosition(m_clientID, m_avoidCarHandel, -1,
                              ms_avoidPosition, simx_opmode_blocking);
      }
  ...
  void thread_gotoTarget() {
    if (m_avoid == true) {
      std::unique_lock<std::mutex>(mlock_avoidPose);
      simxGetObjectPosition(m_clientID, m_avoidCarHandel, -1, ms_avoidPosition,
                            simx_opmode_blocking);
    }
    {
      std::unique_lock<std::mutex>(mlock_pose);
    ...
    while (ros::ok()) {
      ...
      if (m_avoid == true) {
        ...
        // std::cout << "[" << m_name << "]" << " Current distance: " <<
        // distance << std::endl;
      ...
      double angleDistance, distance, phi, phiDistance;
      {
        ...
        // 仿真中小车前向是 y 负方向
        angleDistance = angles::normalize_angle(ms_currentTarget.angular.z -
                                                (ms_orientation[2] - M_PI_2));
        distance = sqrt(pow((ms_currentTarget.linear.x - ms_position[0]), 2.) +
                        pow((ms_currentTarget.linear.y - ms_position[1]), 2.));
        phi = atan2(ms_currentTarget.linear.y - ms_position[1],
                    ms_currentTarget.linear.x - ms_position[0]);
        phiDistance =
            angles::normalize_angle(phi - (ms_orientation[2] - M_PI_2));
      }
      if (distance > 1.0 && abs(phiDistance) > 0.15) {
        double angularVelocity = clip(10 * phiDistance, maxAVelocity);
        // std::cout << "State 1 - distance: " << distance << " phiDistance: "
        // << phi << std::endl;
        setVelocity(0, angularVelocity);
      } else if (distance > 0.5) {
        // std::cout << "State 2 - distance: " << distance << " phi: " << phi <<
        // std::endl; 滑移
        ...
        // std::cout << "State 3 - distance: " << distance << " angleDistance: "
        // << angleDistance << std::endl;
  ...
  bool cb_target(const geometry_msgs::Twist::ConstPtr target) {
    {
      std::unique_lock<std::mutex>(mlock_target);
      double distance = 0.0;
      distance = sqrt(pow(ms_currentTarget.linear.x - target->linear.x, 2.0) +
                      pow(ms_currentTarget.linear.y - target->linear.y, 2.0));
      ...
      ROS_INFO("%s new target: x:%f y:%f phi:%f", m_name.c_str(),
               ms_currentTarget.linear.x, ms_currentTarget.linear.y,
               ms_currentTarget.angular.z);
    }
    m_moveState = true;
    m_first_target = true;
    return true;
  ...
  bool cb_velocity(const geometry_msgs::Twist::ConstPtr msg) {
